...
def interpolate(data_arr, size=1):
    ...
    rpos, cpos = np.where(data_arr == float('-inf'))
    Utils.log("  - Processing empty cells... ")
    Utils.log("  - Total empty cells: ", len(rpos))
...
def smooth_dem(data_arr):
    Utils.log("  - Smoothing DEM... ")
    rows, cols = data_arr.shape
    for k in range(0, 2):
        for i in range(rows):
            for j in range(cols):
                ...
                down = min(i + 1, rows - 1)
                if j != left:
                    total.append(data_arr[i][left])
                if j != right:
                    total.append(data_arr[i][right])
                if i != up:
                    total.append(data_arr[up][j])
                if i != down:
                    total.append(data_arr[down][j])
                ...
                diff2 = pv - total
                diff2 = diff2 > 5
                if diff1.sum() >= 2 or diff2.sum() >= 2:
                    data_arr[i][j] = total.mean()
...
def sub_fun_dem(d_xyz, output_arr_dem, _output_dem_num, _seg_index, _num_w, _num_h, _resolution, _search_length,
                _seg_size, _method):
    ...
    for i in range(lower, upper):
        ...
        # Utils.log influence_right_num, influence_left_num, influence_down_num, influence_up_num
        for rr in range(row - influence_down_num, row + influence_up_num + 1):
            for cc in range(col - influence_left_num, col + influence_right_num + 1):
                if (-1 < rr < _num_h) and (-1 < cc < _num_w):
                    ...
                    if output_arr_dem[_num_h - rr - 1][cc] == float("-inf"):
                        output_arr_dem[_num_h - rr - 1][cc] = 0
                    output_arr_dem[_num_h - rr - 1][cc] += d_xyz[i][2]
                    # output_arr_dem[_num_h - rr - 1][cc] = min(output_arr_dem[_num_h - rr - 1][cc], d_xyz[i][2])


def sub_fun_dsm_dem(d_xyz, _classification, output_arr_dem, _output_dem_num, output_arr_dsm, _seg_index, _num_w, _num_h,
                    _resolution, _search_length, _seg_size, _method):
    # corresponding interval
    lower = _seg_index * _seg_size
    upper = min((_seg_index + 1) * _seg_size, len(d_xyz))
    Utils.log("  - Processing from: ", lower, " to ", upper)
    for i in range(lower, upper):
        ...
        influence_down_num, influence_up_num = max(0, int(math.ceil(delta_y_down / float(_resolution)))), \
                                               max(0, int(math.ceil(delta_y_up / float(_resolution))))
        # Utils.log delta_y_down, delta_y_up
        delta_x_left, delta_x_right = _search_length * 0.5 - cell_relative_x, \
                                      _search_length * 0.5 - (_resolution - cell_relative_x)
        influence_left_num, influence_right_num = max(0, int(math.ceil(delta_x_left / float(_resolution)))), \
                                                  max(0, int(math.ceil(delta_x_right / float(_resolution))))
...
if __name__ == "__main__":
    ...
    parse.add_argument("-fillholeofchm", help="Fill hole within corwn, specify a height difference.", type=float,
                       default=3)
    ...
    Utils.log("  - Total points:", point_number)
    ...
    if args.fill_radius is not None:
        Utils.log("  - Radius filling...: radius =", args.fill_radius)
        NUM = args.fill_num
        ...
        Utils.log("  - Updated points: ", len(delta_xyz))

    # delta_xy = xyz[:, 0:2] - np.array([min_x, min_y])
    Utils.log("  - Start to calculate...")
    ...
    try:
        Utils.log("  - DEM size: ", "Width: ", num_w, " Height: ", num_h)
        Utils.log("  - DEM resolution:", resolution)
        output_dem = np.memmap(dem_out_name, dtype=float, shape=(num_h, num_w), mode='w+')
        output_dem += float('-inf')
        ...
        if has_dsm:
            ...
            output_dsm += float('-inf')
            joblib.Parallel(n_jobs=cpu_cores, max_nbytes=1e4)(
                joblib.delayed(sub_fun_dsm_dem)(delta_xyz, classification, output_dem, output_dem_num, output_dsm, i,
                                                num_w, num_h, resolution, search_length, seg_size, method)
                for i in range(0, seg_num))
            interpolate(output_dsm)
            Utils.saveToHdr(output_dsm, args.dsm, geoTransform)
        else:
            joblib.Parallel(n_jobs=cpu_cores, max_nbytes=1e4)(
                joblib.delayed(sub_fun_dem)(delta_xyz, output_dem, output_dem_num, i,
        ...
        smooth_dem(output_dem)
        Utils.saveToHdr(output_dem, output_img_file, geoTransform)
        ...
        if args.chm is not None and has_dsm:
            ...
            if args.fillholeofchm is not None:
                Utils.log("  - Filling holes of CHM")
                fillHoleofchm(chm, args.fillholeofchm)
            Utils.saveToHdr(chm, args.chm, geoTransform)
        if has_dsm:
            del output_dsm
